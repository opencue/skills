...
metadata:
  version: 1.0.0
  homepage: https://www.npmjs.com/package/opensrc
---
...
Verify with `opensrc --version`.

## Example

Read how Zod implements parsing, then grep its tests:

```bash
cat $(opensrc path zod)/src/types.ts
rg "safeParse" $(opensrc path zod)/src
...
- Use `$(opensrc path <pkg>)` in subshells, path goes to stdout, progress to stderr.
- Prefer auto-detected lockfile versions over `latest` so the source matches what the project runs.
- Don't fetch source when types or docs already answer the question, it costs a clone.
- Cache is global at `~/.opensrc/`; clean with `opensrc clean` when disk matters.
